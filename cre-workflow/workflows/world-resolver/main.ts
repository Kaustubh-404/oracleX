/**
 * OracleX Market Resolver — World Chain CRE Workflow
 *
 * Same AI resolution logic as the Sepolia workflow, but targets
 * World Chain mainnet (chain ID 480, selector "ethereum-mainnet-worldchain-1").
 *
...
const CONTRACT_ADDRESS = (process.env.CONTRACT_ADDRESS ?? "0x266BD36ae57d7803C689A34a97dfE85469295E74") as Address;
const CHAIN_SELECTOR   = EVMClient.SUPPORTED_CHAIN_SELECTORS["ethereum-mainnet-worldchain-1"];
...
export default [
  handler(
    ...
    (runtime: Runtime<unknown>, triggerLog: unknown) => {
      ...
      runtime.log(`[OracleX-World] Resolving market #${marketId}`);

      const market = readMarket(runtime, marketId);

      if (market.outcome !== 0) {
        runtime.log(`[OracleX-World] Market #${marketId} already settled — skipping`);
      ...
      runtime.log(`[OracleX-World] Consensus: outcome=${decision.outcome}, confidence=${decision.confidenceBps}bps`);
      ...
      runtime.log(`[OracleX-World] Settlement written for market #${marketId}`);
